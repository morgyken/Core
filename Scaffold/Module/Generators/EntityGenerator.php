...
namespace Ignite\Core\Scaffold\Module\Generators;
...
class EntityGenerator extends Generator {
    ...
    public function generate(array $entities) {
        ...
        foreach ($entities as $entity) {
            $this->writeFile(
                    $this->getModulesPath("Entities/$entity"), $this->getContentForStub($entityTypeStub, $entity)
            );
            $this->writeFile(
                    $this->getModulesPath("Entities/{$entity}Translation"), $this->getContentForStub("{$entityType}-entity-translation.stub", $entity)
            );
    ...
    private function generateRepositoriesFor($entity) {
        ...
        $entityType = strtolower($this->entityType);
        $this->writeFile(
                $this->getModulesPath("Repositories/{$entity}Repository"), $this->getContentForStub('repository-interface.stub', $entity)
        );
        $this->writeFile(
                $this->getModulesPath("Repositories/Cache/Cache{$entity}Decorator"), $this->getContentForStub('cache-repository-decorator.stub', $entity)
        );
        $this->writeFile(
                $this->getModulesPath("Repositories/{$this->entityType}/{$this->entityType}{$entity}Repository"), $this->getContentForStub("{$entityType}-repository.stub", $entity)
        );
    }
    ...
    private function generateControllerFor($entity) {
        ...
        $this->writeFile(
                $this->getModulesPath("Http/Controllers/Admin/{$entity}Controller"), $this->getContentForStub('admin-controller.stub', $entity)
        );
    }
    ...
    private function generateViewsFor($entity) {
        ...
        foreach ($this->views as $stub => $view) {
            ...
            $this->writeFile(
                    $this->getModulesPath($view), $this->getContentForStub($stub, $entity)
            );
        }
    }
    ...
    private function generateLanguageFilesFor($entity) {
        ...
        $this->writeFile(
                $this->getModulesPath("Resources/lang/en/{$lowerCaseEntity}"), $this->getContentForStub('lang-entity.stub', $entity)
        );
    }
    ...
    private function generateMigrationsFor($entity) {
        ...
        $this->writeFile(
                $this->getModulesPath("Database/Migrations/{$migrationName}"), $this->getContentForStub('create-table-migration.stub', $entity)
        );
        usleep(250000);
        ...
        $this->writeFile(
                $this->getModulesPath("Database/Migrations/{$migrationName}"), $this->getContentForStub('create-translation-table-migration.stub', $entity)
        );
    }
    ...
    private function generateSidebarExtender($entities) {
        if (count($entities) > 0) {
            return $this->writeFile(
                            $this->getModulesPath('Sidebar/SidebarExtender'), $this->getContentForStub('sidebar-extender.stub', 'abc')
            );
        }

        return $this->writeFile(
                        $this->getModulesPath('Sidebar/SidebarExtender'), $this->getContentForStub('empty-sidebar-view-composer.stub', 'abc')
        );
    }
