...
/**
 * Description of CacheNotificationDecorator
 *
 * @author samuel
 */
class CacheNotificationDecorator extends BaseCacheDecorator implements NotificationRepository {
    ...
    public function __construct(NotificationRepository $notification) {
        parent::__construct();
        $this->entityName = 'messaging_notifications';
    ...
    public function latestForUser($userId) {
        return $this->cache->tags($this->entityName, 'global')
                        ->remember("{$this->locale}.{$this->entityName}.latestForUser.{$userId}", $this->cacheTime, function () use ($userId) {
    ...
    public function allForUser($userId) {
        return $this->cache
                        ...
                        ->remember("{$this->locale}.{$this->entityName}.allForUser.{$userId}", $this->cacheTime, function () use ($userId) {
