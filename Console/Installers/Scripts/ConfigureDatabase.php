...
use Ignite\Core\Console\Installers\Writers\EnvFileWriter;
use PDOException;
...
class ConfigureDatabase implements SetupScript {
    ...
    protected function askDatabaseName() {
        do {
            $name = $this->command->ask('Enter your database name', 'collabmed');
            if ($name == '') {
    ...
    protected function askDatabasePassword() {
        $databasePassword = $this->command->ask('Enter your database password (type \'blank\' for no password)', 'secret');
